...
fn sdk_provenance_is_identical_in_build_docs_and_release() {
    ...
    for (name, source) in [("build.rs", build), ("README.md", readme), ("release.yml", workflow)] {
        assert!(source.contains(COMMIT), "{name} does not contain the exact Ghostty commit");
        assert!(source.contains(ZIG), "{name} does not contain the exact Zig version");
    }
}
