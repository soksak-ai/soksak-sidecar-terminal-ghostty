...
//!
//! 데몬 wire 는 엔진-불가지라 ghostty 미러도 같은 tee→feed→rehydrate/StoreBlob 왕복을 탄다.
//!
//! 다섯 결(전부 실 데몬):
...
use soksak_sidecar_terminal_ghostty::daemon::{ControlClient, TeeFrame, TeeStream};
use soksak_sidecar_terminal_ghostty::mirror::Mirror;
use soksak_sidecar_terminal_ghostty::proto;
use soksak_sidecar_terminal_ghostty::service::ServiceClient;
...
fn fresh_home(tag: &str) -> PathBuf {
    ...
    let home = PathBuf::from(std::env::var("HOME").unwrap())
        .join(".soksak-e2e")
        .join(format!("gh-{tag}-{}-{nanos}", std::process::id()));
...
fn sidecar_bin() -> String {
    std::env::var("SOKSAK_SIDECAR_BIN")
        .ok()
        .filter(|p| !p.is_empty())
        .unwrap_or_else(|| env!("CARGO_BIN_EXE_soksak-sidecar-terminal-ghostty").to_string())
}
