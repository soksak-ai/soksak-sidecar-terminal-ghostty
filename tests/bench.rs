...
#[ignore]
fn bench() {
    let report = soksak_contract_terminal::bench::run::<common::SidecarMirror>("soksak-sidecar-terminal-ghostty");
    ...
    if let Ok(dir) = std::env::var("SOKSAK_BENCH_OUT") {
        ...
        std::fs::write(dir.join("ghostty.bench.json"), report.to_json()).expect("write");
    }
