...
target=$1
resolution=$(soksak-sdk validate build-dependencies build-dependencies.json --dependency ghostty-vt-sdk --target "$target") || exit 78
zig_expected=$(printf '%s' "$resolution" | node -e 'let s="";process.stdin.on("data",c=>s+=c).on("end",()=>process.stdout.write(JSON.parse(s).tools.zig))')

case "$(uname -s)-$(uname -m)" in
  Darwin-arm64) host_target=aarch64-apple-darwin; node_platform=darwin; node_arch=arm64 ;;
  Darwin-x86_64)
    if [ "$(sysctl -n hw.optional.arm64 2>/dev/null || true)" = 1 ]; then
      host_target=aarch64-apple-darwin; node_platform=darwin; node_arch=arm64
    else
      host_target=x86_64-apple-darwin; node_platform=darwin; node_arch=x64
    fi
    ;;
...
if [ "$target" != "$host_target" ] || [ -z "$rust_expected" ] || [ "$rust_actual" != "$rust_expected" ] || \
   [ "$rust_host" != "$target" ] || [ "$zig_actual" != "$zig_expected" ] || \
   [ "$node_actual_platform" != "$node_platform" ] || [ "$node_actual_arch" != "$node_arch" ]; then
  printf 'TOOLCHAIN_MISMATCH: target=%s hostTarget=%s rust=%s rustHost=%s zig=%s nodeRuntime=%s/%s; expected rust=%s zig=%s nodeRuntime=%s/%s\n' \
    "$target" "$host_target" "${rust_actual:-missing}" "${rust_host:-unknown}" "${zig_actual:-missing}" \
    "${node_actual_platform:-unknown}" "${node_actual_arch:-unknown}" "${rust_expected:-missing}" "$zig_expected" "$node_platform" "$node_arch" >&2
  exit 78
fi
