...
release: require-tooling require-out verify
	...
	@set -eu; tool="$$(command -v soksak-sdk)"; tooling_root="$$(cd "$$(dirname "$$tool")/.." && pwd -P)"; \
		...
		cp "$$stage/soksak-sidecar-terminal-ghostty"* "$$package/dist/"; \
		...
		archive="$$artifacts/soksak-sidecar-terminal-ghostty-$$version-$(TARGET).tar.gz"; \
...
attest: require-tooling require-out release
	@tool="$$(command -v soksak-sdk)"; tooling_root="$$(cd "$$(dirname "$$tool")/.." && pwd -P)"; \
		...
		--tool "rust=$$(rustc --version | awk '{print $$2}')" --tool "node=$$(node -p 'process.versions.node')" --tool "zig=$$(zig version)"
