...
//! 프론트 터미널 하나다. ghostty 는 응답을 만들 수 있으나 엔진 좌석이 응답 경로를 계수-후-폐기로
//! 막아(write_pty·device_attributes) 나가는 바이트가 0 이고, 삼킨 응답 요구는
//! [`Mirror::suppressed_replies`] 로 관찰만 된다. 복원 시퀀스에는 질의 바이트가 실리지
//! 않는다(이중응답 원천 차단).

use crate::engine::{ColorSnap, Engine, GridCell, ModeSnap};

// ── Mirror — 복원 경로의 단위(ghostty 엔진) ──────────────────────────────────
...
impl Mirror {
    ...
    /// 미러가 삼킨 응답 요구 수(DA1/DSR 등). 관찰 전용 — PTY 로 나가는 경로는 없다.
    /// 엔진 좌석이 응답 경로를 계수-후-폐기로 막고 센 값을 그대로 노출한다.
