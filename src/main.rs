//! soksak-sidecar-terminal-ghostty — 서비스 런타임(SPEC.md §2).
//!
...
use soksak_sidecar_terminal_ghostty::daemon::ControlClient;
use soksak_sidecar_terminal_ghostty::service::{self, new_registry, SpawnCtx};
...
fn main() {
    ...
    let token = match std::fs::read_to_string(
        soksak_sidecar_terminal_ghostty::proto::token_path(&home),
    ) {
    ...
    // 스폰이 새 데몬에 붙는 신선 인스턴스를 띄운다). 폴링 0(블로킹 read).
    {
        ...
        std::thread::spawn(move || {
            let _ = soksak_sidecar_terminal_ghostty::daemon::block_until_daemon_dies(&home);
    ...
    eprintln!(
        ...
        soksak_sidecar_terminal_ghostty::proto::PTYD_PROTOCOL_VERSION,
        std::process::id(),
        soksak_sidecar_terminal_ghostty::proto::service_socket_path(&home).display(),
    );
...
fn redirect_stderr_to_log(home: &std::path::Path) {
    ...
    let path = soksak_sidecar_terminal_ghostty::proto::service_log_path(home);
